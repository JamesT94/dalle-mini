...
#
#         def new_write(b):
#             if getattr(current_thread(), st.report_thread.REPORT_CONTEXT_ATTR_NAME, None):
#                 buffer.write(b)
...
#     """
#     return max(_all_subdirs_of(os.path.join('../yolov5/runs', 'detect')), key=os.path.getmtime)
...
#     """
#     with open(os.path.join('../yolov5/data', 'videos', uploadedfile.name), 'wb') as f:
...
title = st.title('Text-to-Image Generator')
subheader = st.text('What do you want to see? (hopefully a dog)')
...
select_problem = st.sidebar.selectbox('Select model type:',
                                      ('Normal', 'Capgemini Themed', 'Other stuff'))

blank_space_1 = st.text('')

text_prompt = st.text_input("Input some text please")
